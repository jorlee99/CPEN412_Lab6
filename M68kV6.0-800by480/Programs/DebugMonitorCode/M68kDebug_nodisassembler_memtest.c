...
#define addressTestLower 0x08020000
#define addressTestUpper 0x08030000
...
// added function prototypes for memory test functions
//added by Warren
...
void populatememory(int, int);      // populate the memory with selected data length and pattern
void checkmemory(int, int);         // check the memory with selected data length and pattern
void automatictests(void);
void manualtests(void);
void memorywrite(int, int, int);
void memoryread(int, int);
int useraddress(void);
...
//--------------------------------------------------------------------------------------
// supporting functions used for MemoryTest
...
char testdataselection (){
    ...
    if (UserSelection != '1' && UserSelection!='2' && UserSelection!='3'){
        printf("Invalid selection, defaulting to testing with option [1]\n");
        UserSelection = '1';
    }
    return UserSelection;
}
...
void checkmemory(int pattern, int datalength){
    unsigned int* int_mem_pointer;
    unsigned char* char_mem_pointer;
    unsigned short int* shortint_mem_pointer;
...
void automatictests(){
    ...
    else if (usertestdata == '2'){
        selectedPattern = 0xabcdabcd;
    }
    else if (usertestdata == '3'){
        selectedPattern = 0x55555555;
    }
    else if (usertestdata == '4'){
...
void manualtests(){
    ...
    int address; // to be changed later
    char datalength;

    datalength = datatypeselection();
    patternselection = testdataselection();
    address = useraddress(); // to be changed later
    ...
    memorywrite(pattern, address, datalength);
    memoryread(address, datalength);

    return;
}

void memorywrite(int pattern, int address, int datalength){
    unsigned int* int_mem_pointer;
    unsigned char* char_mem_pointer;
    unsigned short int* shortint_mem_pointer;

    if(address >= addressTestUpper || address < addressTestLower){
        printf("Invalid address for write\nWrite Operation Fail\n");
        return;
    }
    else{
        if(datalength ==2 || datalength ==4){ // need to check if the address is even or not
            if (address%2 != 0){ // if the address is not even, fail to write
                printf("Address for word or longword is not even\nWrite Operation Fail\n");
                return;
            }
                ...
                if(datalength ==1){
                    ...
                    *char_mem_pointer = pattern;
                    printf("Byte write operation complete\n");
                }
                else if (datalength ==2){
                    shortint_mem_pointer = address;
                    *shortint_mem_pointer = pattern;
                    printf("Word write operation complete\n");
                }
                else{
                    int_mem_pointer = address;
                    *int_mem_pointer = pattern;
                    printf("Long word write operation complete\n");
                }
            }
        }
    }
...
void memoryread(int address, int datalength){
    unsigned int* int_mem_pointer;
    unsigned char* char_mem_pointer;
    unsigned short int* shortint_mem_pointer;
...
int useraddress(){ // this won't work for now
    int address;
    printf("Enter address to be tested (hex):\n");
    scanf("%x",&address);
    return address;
}
//--------------------------------------------------------------------------------------
// end of functions used in MemoryTest
...
void MemoryTest(void)
{
    ...
    char usertestmode = 'x'; // just a dummy value
    ...
    usertestmode = getchar();

    if(usertestmode == "1"){
        automatictests();
    }
    else if(usertestmode == "2"){
        manualtests();
    }        
