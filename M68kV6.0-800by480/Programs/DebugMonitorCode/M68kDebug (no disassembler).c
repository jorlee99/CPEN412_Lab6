...
// buld up strings for displaying watchpoints
    ...
    //printf("\r\n D7 = $%08X  A7 = $%08X",d7, 0x0C000000) ;
    //printf("\r\n\r\nUSP = $%08X  (A7) User SP", 0x0C000000 ) ;
    //printf("\r\nSSP = $%08X  (A7) Supervisor SP", 0x0C000000) ;
    printf("\r\n SR = $%04X   ",SR) ;
...
void PrintTestTypes(void) {
    printf("\r\nMemory Test Program\r\n") ;
    printf("Options:\r\n") ;
    printf("[1]: Bytes (8-bits)\r\n") ;
    printf("[2]: Words (16-bits)\r\n") ;
    printf("[3]: Long Words (32-bits)\r\n") ;
}

void PrintTestOptions(int option) {
    ...
    if(option == 0){
        ...
        printf("\r\nLong Words (32-bits) Test\r\n") ;
        printf("Options:\r\n") ;
        printf("[1]: 0x1234_5678\r\n") ;
        printf("[2]: 0xAAAA_AAAA\r\n") ;
        printf("[3]: 0xFFFF_FFFF\r\n") ;
        printf("[4]: 0x0000_0000\r\n") ;
    }

}

int TestMemory(unsigned char *memory, unsigned int data, int option){

    unsigned int ReadData;

    unsigned short int *mem_16;
    unsigned int *mem_32;
    ...
    // Test Failed
    if(ReadData != data){
        printf("\r\n[%08x] : %02x , %02x ", memory, ReadData, data) ;
    }

    return (ReadData == data)? 1 : 0 ;
}

void WriteMemory(unsigned char *memory, unsigned int data, int option){
    ...
    if(option == 0){
        *memory = (unsigned char) data;
        ...
        mem_16 = (unsigned short int*)memory;
        *mem_16 = (unsigned short int)data;
    }
    else{
        mem_32 = (unsigned int *)memory;
        *mem_32 = data;
    }
...
void MemoryTest(void)
{
    ...
    int TestOption = -1, DataOption = -1;
    int LowerMemory = -1, UpperMemory = -1;

    int AddressTotal, TestPCTG;
    int unAligenedMemoryUpper, unAligenedMemoryLower;
    int TestTotal, TestPass, TestFail, TestSize;
    int TestOutcome = 0;


    // Memory Testcases
    unsigned int TestCases[3][4] = {{0x12, 0xAA, 0xFF, 0x00}, {0x1234, 0xAAAA, 0xFFFF, 0x0000}, {0x12345678, 0xAAAAAAAA, 0xFFFFFFFF, 0x00000000}};
...
// Get Test Size
    ...
    do{
        ...
        PrintTestOptions(TestOption);

        FlushKeyboard() ;
        DataOption = toupper(_getch()) - 48;

    }while(DataOption<1 || DataOption>4);
    DataOption--;

    // Get Lower Test Address
    do{

        printf("\r\nPlease Enter an address in the range 0x0900_0000 to 0x097F_FFFF\r\n") ;
        printf("Enter lower Address\r\n") ;

        LowerMemory = Get8HexDigits(0) ;

    }while(LowerMemory < 0x09000000 && LowerMemory > 0x097FFFFF);

    // Get Upper Test Address
    do{

        printf("\r\nPlease Enter an address in the range 0x0900_0000 to 0x097F_FFFF\r\n") ;
        printf("Enter Upper Address\r\n") ;

        UpperMemory = Get8HexDigits(0) ;

    }while(UpperMemory > 0x097FFFFF && UpperMemory < 0x09000000);

    // Get Address Size
    AddressTotal = UpperMemory - LowerMemory;

    // Turn Test size into byte size for loop
    TestSize = 1 << (TestOption);

    // Check Unaligend memory inputs
    unAligenedMemoryLower = LowerMemory % 2;
    unAligenedMemoryUpper = UpperMemory % 2;

    TestTotal = 0, TestPass = 0, TestFail = 0;

    printf("Current Test %x\r\n", TestCases[TestOption][DataOption]) ;

    RamPtr = (unsigned char*)LowerMemory;

    // Write Memory
    // Do unaligend byte transfer
    if(unAligenedMemoryLower){
        WriteMemory(RamPtr, TestCases[0][DataOption], 1);
        RamPtr += 1;
    }

    for(; RamPtr <  UpperMemory - unAligenedMemoryUpper; RamPtr+= TestSize){
        WriteMemory(RamPtr, TestCases[TestOption][DataOption], TestOption);

        if(!(RamPtr%10000)){
            TestPCTG = (100*((int)RamPtr - LowerMemory))/(AddressTotal);
            printf("[%08x] : %02x  ", RamPtr, *RamPtr) ;
            printf("PCTG: %d%\r\n", TestPCTG) ;
        }
    }

    if(unAligenedMemoryUpper){
        RamPtr += 1;
        WriteMemory(RamPtr, TestCases[0][DataOption], 1);
    }
    ...
    // Read and Test Memory
    printf("\r\nTesting Data in Memory\r\n") ;
    RamPtr = (int*)LowerMemory;

    if(unAligenedMemoryLower){
        TestOutcome = TestMemory(RamPtr, TestCases[0][DataOption], 0);
    ...
    for(; RamPtr <  UpperMemory - unAligenedMemoryUpper; RamPtr+= TestSize){
        ...
        TestOutcome = TestMemory(RamPtr, TestCases[TestOption][DataOption], TestOption);
    ...
    if(unAligenedMemoryUpper){
        ...
        TestOutcome = TestMemory(RamPtr, TestCases[0][DataOption], 1);
...
void main(void)
{
    ...
    char CourseName[] = "CPEN 412";

    char MarianoStudentName[] = "Mariano Roldan";
    char MarianoStudentNum[] = "77495273";

    char AndrewStudentName[] = "Mr. Andrew Haselhan";
    char AndrewStudentNum[] = "21555065";
